...
import biteNowTracksData from "@/lib/data/biteNowTracks.json";
...
/* ------------------------------------------------------------- Default BITE NOW Tracks */

export const DEFAULT_MEOVV_TRACKS: Track[] = (biteNowTracksData as any[]).map(
  (t) => ({
    title: t.title,
    artist: "MEOVV",
    cover: t.coverArt,
...
function Controls({
  ...
  return (
    <div className="controls">
      ...
      <button className="ctrl" onClick={onPrev} aria-label="Previous" title="Previous Track">
      ...
      <button className="ctrl" onClick={onNext} aria-label="Next" title="Next Track">
        <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
...
/* --------------------------------- Pixel Synchronized Lyrics Component */

function PixelLyricsDisplay({
  lyrics,
  currentTime,
  onSeek,
}: {
  ...
  if (!lyrics || lyrics.length === 0) {
    return (
      <div className="w-full mt-4 p-4 border border-concrete-light/70 bg-void/90 text-center font-pixel text-xs text-fog tracking-widest uppercase">
  ...
  return (
    <div className="w-full mt-4 flex flex-col border border-concrete-light/80 bg-[#07070A]/95 shadow-[0_10px_30px_rgba(0,0,0,0.9)] overflow-hidden">
      {/* Header telemetry in pixel font */}
      <div className="flex items-center justify-between px-3 py-1.5 bg-concrete-dark/90 border-b border-concrete-light/60 font-pixel text-[11px] text-chrome tracking-widest uppercase">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 bg-volt rounded-full animate-ping" />
          <span className="text-volt font-bold">PIXEL LYRIC TELEMETRY</span>
        </div>
        <span className="text-fog">
          LINE {activeIndex >= 0 ? activeIndex + 1 : 0}/{lyrics.length}
        </span>
      </div>

      {/* Auto-scrolling lyrics list in pixel font */}
      <div
        ref={containerRef}
        className="max-h-48 sm:max-h-56 overflow-y-auto p-3.5 space-y-2.5 font-pixel scroll-smooth scrollbar-thin scrollbar-thumb-concrete-light"
      >
        {lyrics.map((line, idx) => {
          ...
          return (
            <div
              ...
              className={`group flex items-start gap-2.5 p-1.5 cursor-pointer transition-all duration-200 border ${
                isActive
                  ? "bg-concrete-light/60 border-volt text-volt font-bold shadow-[0_0_15px_rgba(204,255,0,0.3)] translate-x-1"
                  : isPast
                  ? "border-transparent text-fog/60 hover:text-ash hover:border-concrete-light"
                  : "border-transparent text-fog/40 hover:text-ash hover:border-concrete-light"
              }`}
            >
              {/* Speaker Tag in Pixel Font */}
              {line.speaker ? (
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded-xs tracking-wider uppercase shrink-0 font-pixel border ${
                    isActive
                      ? "bg-volt text-void border-volt font-bold"
                      : "bg-void text-fog border-concrete-light/70 group-hover:text-ash"
                  }`}
                >
                  {line.speaker}
                </span>
              ) : (
                <span className="text-[10px] text-fog/50 font-pixel w-6 text-right shrink-0">
                  {fmt(line.time)}
                </span>
              )}

              {/* Main Lyric Text in Pixel Font */}
              <div className="flex-1 text-xs sm:text-sm tracking-wider uppercase leading-relaxed font-pixel break-words">
                {isActive && <span className="mr-1 text-volt">▶</span>}
                <span>{line.text}</span>
              </div>
            </div>
          );
      ...
      {/* Bottom jump hint */}
      <div className="px-3 py-1 bg-void/90 border-t border-concrete-light/50 text-[10px] font-pixel text-fog/70 flex items-center justify-between">
        <span>CLICK ANY LINE TO JUMP</span>
        <span className="text-volt">MEOVV ARCHIVE</span>
      </div>
    </div>
  );
}
...
export function MusicPlayer({
  tracks = DEFAULT_MEOVV_TRACKS,
  crossOrigin,
  showLyrics = true,
}: MusicPlayerProps) {
  const player = useAudioPlayer(tracks);
  const [isZoomed, setIsZoomed] = useState(false);
  const [lyricsOpen, setLyricsOpen] = useState(showLyrics);
  ...
  return (
    ...
    >
      ...
      {/* Header Pill & Lyrics Toggle */}
      <div className="w-full flex items-center justify-between pb-3 mb-1 border-b border-concrete-light/50">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-volt animate-pulse shadow-[0_0_8px_#CCFF00]" />
          <span className="font-pixel text-[11px] text-chrome font-bold tracking-widest uppercase">
            MEOVV · BITE NOW EP
          </span>
        </div>
        ...
        <button
          ...
          className={`px-2 py-0.5 rounded-none font-pixel text-[10px] tracking-wider uppercase border transition-all cursor-pointer ${
            lyricsOpen
              ? "bg-volt text-void border-volt font-bold shadow-[0_0_10px_rgba(204,255,0,0.4)]"
              : "bg-void text-fog border-concrete-light hover:text-white"
          }`}
          title="Toggle Pixel Synced Lyrics"
        >
          {lyricsOpen ? "LYRICS [ON]" : "LYRICS [OFF]"}
        </button>
      </div>

      {/* Spinning Vinyl Disc with Zoom Interaction */}
      <Disc
        layers={layers}
      ...
      {/* Improvised Real-Time Synchronized Lyrics in Pixel Font */}
      {lyricsOpen && (
        <PixelLyricsDisplay
